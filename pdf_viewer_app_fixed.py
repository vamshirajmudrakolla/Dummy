...
class PDFViewerPane:
    ...
    Runs in a worker thread and always returns a tuple:
    (pdf_document_obj, words_data_obj, temp_pdf_path_used, error_msg)
    """
    ...
    try:
        ...
        # Convert Word/RTF/TXT to a temporary PDF first
        if original_ext in ('.doc', '.docx', '.rtf', '.txt'):
            print(f"Attempting to convert {original_ext} file to PDF...")
        ...
        # Open the (possibly converted) PDF and extract words
        pdf_document_obj = fitz.open(file_path)
        words_data_obj = extract_words_with_styles(pdf_document_obj)

        # Clipboard helper output is also a temp file we may need to clean later
        if "clipboard_temp_" in file_path:
            temp_pdf_path_used = file_path

        return pdf_document_obj, words_data_obj, temp_pdf_path_used, None

    except Exception as e:
        # DO NOT re-raise; fail gracefully so the GUI can update the pane
        print(f"Error in load_pdf_internal: {e}")
        traceback.print_exc()

        # Best-effort cleanup of any temporary PDF we created
        ...
        # Close partially opened document if any
        if pdf_document_obj:
            try:
