...
# ---- Quiet mode (suppress noisy prints; keep errors/warnings) ----
QUIET_MODE = True
if QUIET_MODE:
...
TEMP_PDF_DIR = os.path.join(tempfile.gettempdir(), "pdf_viewer_temp_pdfs")
...
class PDFViewerPane:
    ...
    def goto_page(self, page_index:int):
        """Scroll this pane so that page_index (0-based) is at the top."""
        try:
            if not self.pdf_document or self.pdf_document.is_closed:
                return
            page_index = max(0, min(page_index, self.pdf_document.page_count - 1))
            # Ensure layout info is available
            if page_index not in self.page_layout_info:
                self.calculate_layout_for_all_pages()
            y_offset_doc = self.page_layout_info.get(page_index, {}).get("y_start_offset", 0)
            ...
            x_start_frac = self.canvas.xview()[0] if self.canvas.xview() else 0.0
            x_scroll_pixels = x_start_frac * total_width_at_zoom
            y_scroll_pixels = y_offset_doc * self.zoom_level
            self._apply_scroll(x_scroll_pixels, y_scroll_pixels)
        except Exception as e:
            print(f"goto_page error in pane {self.pane_id}: {e}")
    # -------------------------------------------------------------------------
...
class PDFViewerApp:
    ...
    def setup_ui(self):
        ...
        ttk.Label(control_frame, text="  Page:").pack(side=tk.LEFT, padx=(10,0))
        self.page_jump_entry = ttk.Entry(control_frame, width=6, textvariable=self.page_jump_var)
        self.page_jump_entry.pack(side=tk.LEFT, padx=(4,0))
        def _do_jump(event=None):
            v = self.page_jump_var.get().strip()
            if not v:
                return
            try:
                n0 = max(0, int(v) - 1)
            except ValueError:
                messagebox.showerror("Go to page", f"'{v}' is not a number")
                return
            try:
                self.pane1.goto_page(n0)
            except Exception:
                pass
            try:
                self.pane2.goto_page(n0)
            except Exception:
                pass
        self.page_jump_entry.bind("<Return>", _do_jump)
        ttk.Button(control_frame, text="Go", command=_do_jump).pack(side=tk.LEFT, padx=(4,6))
