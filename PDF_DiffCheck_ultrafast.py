...
def quick_page_hash(page):
    t = page.get_text("text", sort=True)
    t = " ".join(t.split()).lower()
    return hashlib.sha1(t.encode("utf-8")).hexdigest()

def build_page_hashes(doc):
    return [quick_page_hash(doc.load_page(i)) for i in range(doc.page_count)]

@lru_cache(maxsize=8192)
def _extract_words_for_page(doc_id, page_index):
    pdf_document, file_path = doc_id
...
def extract_words_with_styles(pdf_document):
    """
    FAST: Instead of extracting all words for all pages, defer extraction until needed.
    Each entry is None initially; use _extract_words_for_page to get actual words later.
    """
    if not pdf_document or pdf_document.is_closed:
        return []
    return [None] * pdf_document.page_count
